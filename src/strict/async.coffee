...
matchVertex = ( graph, talos ) ->
  ...
  if !vertex?
    talos.throw Errors.InvalidState.make "talos state is not
      in graph"
  vertex
...
matchEdge = ( vertex, talos, transforms ) ->
  for edge in vertex.edges
    if ( await edge.accept talos, transforms... ) == true
      return edge
  talos.throw Errors.MissingTransition.make "no edge matches transforms"

run = ( edge, talos, transforms ) ->
  if edge.run?
    try
      await edge.run talos, transforms...
    catch error
...
move = ( edge, talos, transforms ) ->
  try
    await edge.move talos, transforms...
  catch error
